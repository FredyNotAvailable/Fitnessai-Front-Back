...
// Enum para categorías (inglés en minúscula => español)
const Categoria = {
  chest: "Pecho",
  back: "Espalda",
  arm: "Brazo",
...
function traducirCategoria(cat: string) {
  return (Categoria as any)[cat] || cat;
}

function traducirDia(dia: string) {
  return (Dia as any)[dia] || dia;
}

function traducirRutina(routine: any) {
  if (!routine) return routine;
  return {
    ...routine,
    day: traducirDia(routine.day),
    categories: routine.categories?.map(traducirCategoria),
    notes: routine.notes,
...
export function Routine() {
  ...
  useEffect(() => {
    if (user?.uid) {
      loadByUserId(user.uid).then((routines) => {
        if (routines.length) setSelectedDay(traducirDia(routines[0].day));
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (updatedRoutines.length) setSelectedDay(traducirDia(updatedRoutines[0].day));
      else setSelectedDay(null);
    } catch (e) {
  ...
  const translatedRoutines = routineDaysByUser.map(traducirRutina);
  const days = translatedRoutines.map((r) => r.day);
  const routineForDay = translatedRoutines.find((r) => r.day === selectedDay);
    ...
    >
      ...
      <Box flex="none" mb={4}>
        ...
        <Heading size="lg" mb={1} color="primary.900" textAlign="center">
          Rutina del día: {routineForDay?.day}
        </Heading>

        {/* Mostrar categorías, duración y notas */}
        <Box textAlign="center" mb={4}>
          {(routineForDay?.categories?.length ?? 0) > 0 && (
            <Text
              fontWeight="semibold"
              fontSize="md"
        ...
        <Box overflowX="auto" whiteSpace="nowrap">
          <ButtonGroup isAttached variant="outline" size="md">
            {days.map((day, index) => {
              const isSelected = day === routineForDay?.day;
              return (
                <Button
                  key={day}
      ...
      {/* Lista de ejercicios */}
      <RoutineExerciseList exercises={routineForDay?.exercises || []} />
    </Box>
  );
}
